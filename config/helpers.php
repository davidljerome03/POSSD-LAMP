...
/**
 * Find if a user with the given $login exists in database $db
 */
function userExists($login, $db) {
	$fstmt = $db->prepare("SELECT COUNT(*) FROM Users WHERE Login=:login");
	$fstmt->execute([':login' => $login]);
	$found = $fstmt->fetch();
	return $found["COUNT(*)"];
}
...
/**
 * Send the JSON response that should be recieved when the user logs in - used for allowing existing users to
